            ...
            out.value = 'Só numeros entre 0s e 7s';
                ...
                if(castNumb.length > 4  &&  castNumb.length%4 != 0){
                  for(let ccc = 0; ccc < arrayToHex.length; ccc++){
                    ...
                    if(arrayToHex.toReversed()[1] > 0 && arrayToHex.toReversed()[0] == 0){
                        arrayToHex.pop();//remove the zero when the most left number is 0(after all calculations like *0*00111010 or other number)
                        //or other number in hexa to bin like 321(16)  = 001100100001(2) the first number is 0
